...
export const editProductModal = async (product: IProduct): Promise<void> => {
    ...
    <div class="leftModal">
        <h2>Modify Item</h2>
        <input type="text" name="code" required placeholder="Code *" value="${product.id}">
    ...
    <div class="rightModal">
        <img class="currentImg" src="${product.image}">
    ...
    modal.addEventListener("submit", async (e: SubmitEvent) => {
        ...
        if (image.name) {
            ...
            product.image = URL.createObjectURL(image) as string;
        } else if (currentImg.src !== product.image) {
